...
const Grid = struct {
    ...
    inline fn setAt(self: *Grid, offset: usize, value: bool) void {
        self.grid[offset] = value;
    }
    ...
    inline for (&[_]isize{ -1, 0, 1 }) |xd| {
        inline for (&[_]isize{ -1, 0, 1 }) |yd| {
            ...
            if (!(target_x == x and target_y == y)) {
                if (self.grid.at(self.posToOffset(@intCast(target_x), @intCast(target_y)))) {
                    count += 1;
                }
            }
        }
...
pub fn live(self: *Game) void {
    ...
    while (it.next()) |cell| {
        ...
        switch (neighbors) {
            0...1 => buf.setAt(offset, false),
            2 => buf.setAt(offset, cell.alive),
            3 => buf.setAt(offset, true),
            else => buf.setAt(offset, false),
        }
    }
}
...
pub const Iterator = struct {
    ...
    pub fn next(self: *@This()) ?Item {
        if (self.offset >= self.grid.grid.len) return null;
        defer self.offset += 1;
