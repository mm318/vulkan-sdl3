...
pub fn load_image_from_file(engine: *Engine, filepath: []const u8) !Engine.AllocatedImage {
    ...
    if (image_data == null) {
        std.log.err("{s}", .{c.stbi.failure_reason()});
    ...
    const staging_buffer = engine.create_buffer(image_size, c.vk.BUFFER_USAGE_TRANSFER_SRC_BIT, c.vma.MEMORY_USAGE_CPU_ONLY);
